...
96 billion operations per second and a deliberately fine-grained memory
organization: 9216 18-bit words of RAM and 9216 18-bit words of ROM
distributed across the array, which means 64 RAM words and 64 ROM words
local to each node. This makes the implementation burden tangible:
behavior blocks must be small, local, and communication-aware rather than
written as ordinary memory-rich routines. The same documents describe
application-dependent power scaling from suspended microwatt operation to
hundreds of milliwatts under heavier activity
\citep{greenarrays2026ga144a12,cartheur2026pb001}. That
