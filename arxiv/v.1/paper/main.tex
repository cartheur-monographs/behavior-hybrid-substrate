...
behavior together rather than apart. The second paper moved from historical
reconstruction to preserved robotic software and argued that Sony's
\texttt{ERS-111} \texttt{R-CODE} sample set is organized by a compact
behavioral state vocabulary centered on initialization, sensing, action,
synchronization, and recovery \citep{tucker2026behavioral}. The present
paper takes the next step. It asks how such a vocabulary can be made
executable on a distributed substrate instead of remaining a descriptive
layer over legacy code.
...
eventually embodied in hardware.
